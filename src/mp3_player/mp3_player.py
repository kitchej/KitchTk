...
class MP3Player(tk.Frame):
    def __init__(self, text_font : tuple[str, int] = None, *args, **kwargs):
        tk.Frame.__init__(self, *args, **kwargs)

        # Icon paths - Feel free to point to your own icon set if desired
        rewind = "icons/rewind_bootstrap.png"
        stop = "icons/stop_bootstrap.png"
        play = "icons/play_bootstrap.png"
        pause = "icons/pause_bootstrap.png"
        fast_forward = "icons/fast_forward_bootstrap.png"
        ...
        # Update interval for the playhead (in milliseconds)
        self.playhead_update_interval = 500
        ...
        self.rewind_icon = ImageTk.PhotoImage(image=Image.open(rewind).resize(self.icon_size))
        self.stop_icon = ImageTk.PhotoImage(image=Image.open(stop).resize(self.icon_size))
        self.play_icon = ImageTk.PhotoImage(image=Image.open(play).resize(self.icon_size))
        self.pause_icon = ImageTk.PhotoImage(image=Image.open(pause).resize(self.icon_size))
        self.fast_forward_icon = ImageTk.PhotoImage(image=Image.open(fast_forward).resize(self.icon_size))


        self.file_lab = tk.Label(self, textvariable=self.filename, font=self.font)
        self.playhead_frame = tk.Frame(self)
        self.playhead = ttk.Scale(self.playhead_frame, orient=tk.HORIZONTAL, state=tk.DISABLED, takefocus=0)
        self.time_remain_lab = tk.Label(self.playhead_frame, textvariable=self.time_remain, font=self.font)
        self.total_time_lab = tk.Label(self.playhead_frame, textvariable=self.total_time, font=self.font)
        self.controls_frame = tk.Frame(self)
        self.rewind_butt = ttk.Button(self.controls_frame, image=self.rewind_icon, command=self.rewind, state=tk.DISABLED)
        self.stop_butt = ttk.Button(self.controls_frame, image=self.stop_icon, command=self.stop, state=tk.DISABLED)
        self.play_butt = ttk.Button(self.controls_frame, image=self.play_icon, command=self.play, state=tk.DISABLED)
        self.pause_butt = ttk.Button(self.controls_frame, image=self.pause_icon, command=self.pause, state=tk.DISABLED)
        self.forward_butt = ttk.Button(self.controls_frame, image=self.fast_forward_icon, command=self.fast_forward, state=tk.DISABLED)
        ...
        self.rewind_butt.pack(side=tk.LEFT, padx=(0, 0))
        self.stop_butt.pack(side=tk.LEFT, padx=0)
        self.play_butt.pack(side=tk.LEFT, padx=0)
        self.pause_butt.pack(side=tk.LEFT, padx=0)
        self.forward_butt.pack(side=tk.LEFT, padx=(0, 0))
    ...
    def _advance_playhead(self, *args):
        if not self.user_updating_playhead:
    ...
    def rewind(self, interval=4): # interval is in seconds
        self.playback_obj.seek(self.playback_obj.curr_pos - interval)
        if not self.playback_obj.playing:
            self.play()

    def fast_forward(self, interval=4): # interval is in seconds
        self.playback_obj.seek(self.playback_obj.curr_pos + interval)
        if not self.playback_obj.playing:
            self.play()
